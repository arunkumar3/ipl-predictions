      ...
      .on(
        ...
        (payload) => {
          if (payload.eventType === 'INSERT' || payload.eventType === 'UPDATE') {
            setPredictions((prev) => ({
              ...prev,
              [payload.new.match_number]: payload.new,
            }));
          } else if (payload.eventType === 'DELETE') {
            setPredictions((prev) => {
              const next = { ...prev };
              delete next[payload.old.match_number];
              return next;
            });
          }
        }
    ...
    async (matchNumber, team) => {
      ...
      const { error: upsertError } = await supabase
        ...
        .upsert(
          ...
          { onConflict: 'user_name,match_number' }
        );

      if (upsertError) {
        // Rollback
        if (previous) {
          setPredictions((prev) => ({ ...prev, [matchNumber]: previous }));
        } else {
          setPredictions((prev) => {
            const next = { ...prev };
