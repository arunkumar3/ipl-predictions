      ...
      <div className="px-4 pt-3 pb-3">
        ...
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            <span className="font-mono-num text-[11px] font-bold" style={{ color: '#8890A6' }}>
              #{match.match_number}
            </span>
            <span
        ...
        <div className="grid items-center" style={{ gridTemplateColumns: '1fr auto 1fr' }}>
          <div className="flex justify-center">
            <TeamBadge team={match.team1} size="md" selected={predictedTeam === match.team1} onClick={() => handlePredict(match.team1)} disabled={locked} />
          </div>
          <div className="flex flex-col items-center px-2">
            <span className="text-xl font-black tracking-tight" style={{ color: '#E8458B' }}>VS</span>
          </div>
          <div className="flex justify-center">
            <TeamBadge team={match.team2} size="md" selected={predictedTeam === match.team2} onClick={() => handlePredict(match.team2)} disabled={locked} />
          </div>
        </div>
        ...
        {!locked && !predictedTeam && (
          <p className="text-center text-[11px] font-semibold mt-2" style={{ color: '#1B2A6B' }}>
        ...
        {isCompleted && match.winner && predictedTeam && (
          <div
            ...
            style={{ backgroundColor: isCorrect ? '#E8F8EE' : '#FEE7E7' }}
          >
            <span className="text-xs font-semibold" style={{ color: isCorrect ? '#16A34A' : '#E24B4A' }}>
              {isCorrect ? `You predicted ${predictedTeam} — Correct!` : `You predicted ${predictedTeam} — Wrong`}
            </span>
            {isCorrect && (
              <span className="font-mono-num text-xs font-bold px-2 py-0.5 rounded-full" style={{ backgroundColor: '#E8F8EE', color: '#16A34A', border: '1px solid #B6E8C8' }}>
                +{points}
              </span>
            )}
          </div>
  ...
  if (locked) {
    return (
      <div className="flex justify-center mt-2">
        <span className="text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wide" style={{ backgroundColor: '#FEE7E7', color: '#E24B4A' }}>
          LOCKED
        </span>
      </div>
    );
  ...
  return (
    <div className="flex justify-center mt-2">
      <span
        ...
        style={{
          backgroundColor: isUrgent ? '#FEE7E7' : '#EEF3FF',
          color: isUrgent ? '#E24B4A' : '#1B2A6B',
        }}
      >
...
function StatusBadge({ status, locked, isLive }) {
  if (isLive) {
    return (
      <span className="status-live flex items-center gap-1.5 text-[9px] font-extrabold px-2.5 py-[3px] rounded-md uppercase" style={{ backgroundColor: '#FEE7E7', color: '#E24B4A', letterSpacing: '0.5px' }}>
  ...
  if (status === 'completed') {
    return <span className="text-[9px] font-extrabold px-2.5 py-[3px] rounded-md uppercase" style={{ backgroundColor: '#E8F8EE', color: '#16A34A', letterSpacing: '0.5px' }}>COMPLETED</span>;
  }
  if (locked) {
    return <span className="text-[9px] font-extrabold px-2.5 py-[3px] rounded-md uppercase" style={{ backgroundColor: '#FEE7E7', color: '#E24B4A', letterSpacing: '0.5px' }}>LOCKED</span>;
  }
